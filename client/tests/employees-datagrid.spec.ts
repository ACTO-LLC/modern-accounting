...
test.describe('Employees DataGrid', () => {
  test('should display employees in DataGrid', async ({ page }) => {
    ...
    await expect(page.locator('.MuiDataGrid-columnHeader').filter({ hasText: /Employee.*#/i })).toBeVisible();
  ...
  test('should sort employees by clicking column header', async ({ page }) => {
    await page.goto('/employees');
    await page.waitForSelector('.MuiDataGrid-root', { timeout: 10000 });
  ...
  test('should filter employees using column filter', async ({ page }) => {
    await page.goto('/employees');
    await page.waitForSelector('.MuiDataGrid-root', { timeout: 10000 });

    const hasRows = await page.locator('.MuiDataGrid-row').first().isVisible({ timeout: 5000 }).catch(() => false);
    test.skip(!hasRows, 'No employee data to filter');
