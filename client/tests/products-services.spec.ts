...
test.describe('Products & Services Management', () => {
  test('should create and edit a product/service', async ({ page }) => {
    ...
    await page.getByLabel('Sales Price').fill('100');
    await page.getByLabel('Purchase Cost').fill('50');
  ...
  test('should create inventory product with asset account', async ({ page }) => {
    ...
    await page.getByLabel('Name').fill(inventoryName);

    // Select Type (MUI select) - use combobox role to avoid strict mode with listbox
    await page.getByRole('combobox', { name: /Type/ }).click();
    await page.getByRole('option', { name: 'Inventory Product', exact: true }).click();

    // Verify Inventory Asset Account field appears
    await expect(page.getByLabel('Inventory Asset Account')).toBeVisible();

    await page.getByLabel('SKU').fill(`INV-${timestamp}`);
    await page.getByLabel('Sales Price').fill('75');
    await page.getByLabel('Purchase Cost').fill('40');
    ...
    await expect(page).toHaveURL(/\/products-services$/, { timeout: 15000 });
    await page.goto(`/products-services/${createdId}/edit`);
    await expect(page.getByLabel('Name')).toHaveValue(inventoryName);
    await expect(page.locator('input[name="Type"]')).toHaveValue('Inventory');
  });
  ...
  test('should validate required fields', async ({ page }) => {
    ...
    // Verify error message appears (helperText on MUI TextField)
  ...
  test('should validate negative prices', async ({ page }) => {
    ...
    await page.getByLabel('Name').fill(`Test Negative ${timestamp}`);
    await page.getByLabel('Sales Price').fill('-10.00');

    await page.getByRole('button', { name: 'Save Product/Service' }).click();

    // Verify error message appears (helperText on MUI TextField)
    await expect(page.getByText('Sales price cannot be negative')).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should show type-specific help text', async ({ page }) => {
    ...
    // Service is the default type, verify its help text
    await expect(page.getByText('Services you provide to customers')).toBeVisible();

    // Check Non-Inventory help text - use combobox role to avoid strict mode violation
    await page.getByRole('combobox', { name: /Type/ }).click();
    await page.getByRole('option', { name: 'Non-Inventory Product' }).click();
